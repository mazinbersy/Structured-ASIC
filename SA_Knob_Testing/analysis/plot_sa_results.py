...
# CSV_PATH = "analysis_results/sa_refine_temp_results.csv"
# OUT_PNG = "analysis_results/sa_refine_temp_knob_analysis.png"
...
def compute_pareto_front(points):
    """
    points = [(runtime, hpwl, idx), ...]
    Returns indices of the Pareto frontier
    minimizing HPWL for increasing runtime.
    """
...
# Label each point with swept parameters
labels = np.array([
    f"T0={r['initial_temp']}, Pref={r['prob_refine']}"
    for r in rows
])

# Remove outliers (> 2× median runtime)
med = np.median(runtimes)
keep = runtimes < (2 * med)
...
plt.ylabel("Final HPWL (µm)")
plt.title(
    "SA Parameter Sweep: Initial Temperature vs Refine/Explore Ratio\n"
    "Constants: α=0.92, N=200, single greedy seed"
)

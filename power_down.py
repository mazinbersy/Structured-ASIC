...
def run_power_down_eco(
    ...
    """
    Run Power-Down ECO flow using pre-built databases.

    This is the PRIMARY interface - takes pre-built logical_db and fabric_db.
    ...
    Example:
        ...
        >>> from power_down_eco import run_power_down_eco
        >>>
        >>> logical_db, _ = parse_design_json("designs/6502_mapped.json")
        >>> fabric_db = build_fabric_db("fabric/fabric_cells.yaml",
        ...                              "fabric/pins.yaml",
        ...                              "fabric/fabric.yaml")
        >>> placement_map = load_placement_mapping("placement.map")
        >>>
        >>> updated_db, report = run_power_down_eco(
        ...     logical_db=logical_db,
        ...     fabric_db=fabric_db,
...
if __name__ == "__main__":
    if len(sys.argv) < 5:
        print(
            "Usage: python power_down_eco.py <design_mapped.json> <fabric_cells.yaml> <pins.yaml> <fabric.yaml> [placement.map]")
        print()
        print("Arguments:")

...
def plot_fabric_ground_truth(
    ...
    Automatically extracts pins from fabric_db if not provided.
    """
    ...
    ax.add_patch(patches.Rectangle((dx0, dy0), dx1 - dx0, dy1 - dy0, fill=False, lw=2, label="die"))
    ax.add_patch(patches.Rectangle((cx0, cy0), cx1 - cx0, cy1 - cy0, fill=False, lw=2, ls='--', label="core"))
    ...
    if pin_list:
        ...
        ax.scatter(pxs, pys, s=18, marker='o', color='red', zorder=10)
        for p in pin_list:
            name = p.get("name")
            if not name:
                continue
            show_every = max(1, len(pin_list) // 80)
            if (hash(name) & 0xFFFF) % show_every == 0:
                ax.text(p["x"], p["y"] + 0.02 * (ymax - ymin + 1), name, fontsize=6)

    # Legend
    handles = []
    for t, idx in list(type_to_index.items())[:20]:
        c = cmap(idx % cmap.N)
        handles.append(patches.Patch(facecolor=c, alpha=alpha, label=t))
    if handles:
        ax.legend(handles=handles, title="Slot types", loc='upper right', fontsize=8)
...
if __name__ == "__main__":
    ...
    plot_fabric_ground_truth(fabric_db, show=True)
